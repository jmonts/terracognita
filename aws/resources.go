...
func ec2TransitGatewayPrefixListReferences(ctx context.Context, a *aws, resourceType string, filters *filter.Filter) ([]provider.Resource, error) {
	transitGatewayPrefixListReferences, err := a.awsr.GetTransitGatewayPrefixListReference(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	for _, i := range transitGatewayPrefixListReferences {

		r, err := initializeResource(a, fmt.Sprintf("%s_%s", *i.TransitGatewayRouteTableId, *i.PrefixListId), resourceType)
		if err != nil {
			return nil, err
		}
